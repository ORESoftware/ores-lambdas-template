...
/// appropriate for portable/stdin hosts with no stronger invocation metadata.
...
/// Parse + dispatch while making adapter provenance authoritative. The input value is rebuilt into
/// a fresh owned object rather than mutating the decoded map, preserving the functional boundary
/// while preventing caller-controlled provider/request metadata from spoofing host context.
