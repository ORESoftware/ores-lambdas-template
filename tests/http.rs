...
fn app() -> axum::Router { router(HttpConfig { provider: Provider::Local }) }

#[tokio::test]
async fn probes_and_invoke() {
    let res = app().oneshot(Request::get("/healthz").body(Body::empty()).unwrap()).await.unwrap();
    assert_eq!(res.status(), StatusCode::OK);
    let env = format!(r#"{{"provider":"gcp-cloud-run","requestId":"r-9","command":{{"schemaVersion":"{SCHEMA_VERSION}","operation":"version"}}}}"#);
    let res = app().oneshot(Request::post("/invoke").header("content-type", "application/json").body(Body::from(env)).unwrap()).await.unwrap();
    assert_eq!(res.status(), StatusCode::OK);
...
async fn bad_envelope_is_400_and_request_id_comes_from_header() {
    let res = app().oneshot(Request::post("/invoke").header("x-request-id", "trace-42").body(Body::from("{}")).unwrap()).await.unwrap();
...
async fn oversized_body_is_413() {
    ...
    let res = app().oneshot(Request::post("/invoke").body(Body::from(big)).unwrap()).await.unwrap();
    assert_eq!(res.status(), StatusCode::PAYLOAD_TOO_LARGE);
}

#[test]
fn provider_detection_follows_platform_env() {
    assert_eq!(detect_provider(|k| (k == "FUNCTIONS_CUSTOMHANDLER_PORT").then(|| "7071".into())), Provider::AzureFunctions);
    assert_eq!(detect_provider(|k| (k == "K_SERVICE").then(|| "svc".into())), Provider::GcpCloudRun);
